...
public class GameProcessService {
    ...
    public CardResponseMessage requestCards(CardRequestMessage requestMessage, String roomId, String receiptSessionID, String requestingSessionID) throws ObjectNotFoundException {
        ...
        if (potentialChestCardValue != null)
            sendGameProcessEvent(roomId, ChestCompletedEvent.builder().completedChestCardValue(potentialChestCardValue)
                    ...
                    .build());
        if (receiptPlayer.getCards().size() < 4)
            while (receiptPlayer.getCards().size() != 4 && gameRoom.getDeck().size() != 0) {
